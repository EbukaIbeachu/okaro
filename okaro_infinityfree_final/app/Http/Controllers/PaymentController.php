...
class PaymentController extends Controller
{
    ...
    {
        $payments = Payment::with(['rent.tenant', 'rent.unit.building'])
            ->latest('payment_date')
            ->paginate(20);
    ...
    public function create(Request $request)
    {
        ...
        $rents = Rent::with(['tenant', 'unit.building'])->whereHas('tenant', function($q) {
            $q->where('active', true);
        })->get();
        
        $selectedRent = $rentId ? Rent::find($rentId) : null;
