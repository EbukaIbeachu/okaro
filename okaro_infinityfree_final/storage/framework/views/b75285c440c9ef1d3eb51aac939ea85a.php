...
<head>
    ...
    <title>Tenancy Agreement - <?php echo e($rent->tenant->full_name); ?></title>
    <style>
        body {
...
<div class="container">
    ...
    <div class="section">
        THIS TENANCY AGREEMENT is made this <span class="bold"><?php echo e(now()->format('jS')); ?></span> day of <span class="bold"><?php echo e(now()->format('F, Y')); ?></span>.
    </div>
    ...
    <div class="section">
        <span class="bold uppercase"><?php echo e($rent->tenant->full_name); ?></span> of <?php echo e($rent->unit->building->address); ?> (hereinafter referred to as <span class="bold">"THE TENANT"</span> which expression shall where the context so admits include his/her heirs, personal representatives, and assigns) of the <span class="bold">OTHER PART</span>.
    </div>

    <div class="section">
        <span class="bold">WHEREAS:</span>
        <ol class="clause-list">
            <li>The Landlord is the beneficial owner of the property known as <span class="bold"><?php echo e($rent->unit->building->name); ?>, Unit <?php echo e($rent->unit->unit_number); ?></span> situate at <span class="bold"><?php echo e($rent->unit->building->address); ?></span> (hereinafter referred to as <span class="bold">"THE DEMISED PREMISES"</span>).</li>
            <li>The Landlord has agreed to let and the Tenant has agreed to take the Demised Premises for a term of <span class="bold"><?php echo e(\Carbon\Carbon::parse($rent->start_date)->diffInMonths(\Carbon\Carbon::parse($rent->end_date))); ?> Months / 1 Year</span> commencing on <span class="bold"><?php echo e(\Carbon\Carbon::parse($rent->start_date)->format('jS F, Y')); ?></span> and ending on <span class="bold"><?php echo e(\Carbon\Carbon::parse($rent->end_date)->format('jS F, Y')); ?></span>.</li>
        </ol>
    </div>
    ...
    <div class="section">
        ...
        IN CONSIDERATION of the Rent sum of <span class="bold">₦<?php echo e(number_format($rent->annual_amount, 2)); ?> (<?php echo e(ucwords((new NumberFormatter('en', NumberFormatter::SPELLOUT))->format($rent->annual_amount))); ?> Naira Only)</span> per annum paid by the Tenant to the Landlord (the receipt whereof the Landlord hereby acknowledges), the Landlord hereby lets the Demised Premises to the Tenant.
    </div>

    <div class="section">
        2. <span class="bold">THE TENANT COVENANTS WITH THE LANDLORD AS FOLLOWS:</span>
        <ol class="sub-clause">
            <li>To pay the reserved rent in advance on the due date without any deduction or set-off. <span class="bold">Rent is due on the <?php echo e(\Carbon\Carbon::create(2000, 1, $rent->due_day)->format('jS')); ?> day of each month/year.</span></li>
    ...
    <div class="signature-section">
        ...
        <div class="signature-row">
            <div class="signature-box">
                ...
                ______________________________<br>
                <span class="bold"><?php echo e($rent->tenant->full_name); ?></span>
            </div>
            <div class="witness-box">
                IN THE PRESENCE OF:<br><br>
                <span class="bold">Noyb Fundamentals</span><br>
...
</html><?php /**PATH C:\Program Files\EasyPHP-Devserver-17\eds-www\okaro\resources\views/rents/agreement.blade.php ENDPATH**/ ?>